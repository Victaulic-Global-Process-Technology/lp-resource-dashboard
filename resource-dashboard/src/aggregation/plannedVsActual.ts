...
export async function computeMonthlyCategoryTotals(
  ...
  const filteredPlanned = projectFilter
    ? allocations.filter(a =>
        getProjectParent(a.project_id) === projectFilter || a.project_id === projectFilter
      )
    : allocations;

  // Get all unique months
  const allMonths = new Set([
    ...filteredPlanned.map(a => a.month),
  ...
  for (const month of allMonths) {
    const monthPlanned = filteredPlanned.filter(a => a.month === month);
    const monthActual = actualHours.filter(a => a.month === month);
    const monthLabTech = labTechHours.filter(l => l.month === month);

    // Planned totals by type (sum planned_hours from allocations)
    const planned_npd = monthPlanned
      .filter(a => projectMap.get(a.project_id)?.type === ProjectType.NPD)
      .reduce((sum, a) => sum + a.planned_hours, 0);

    const planned_sustaining = monthPlanned
      .filter(a => projectMap.get(a.project_id)?.type === ProjectType.Sustaining)
      .reduce((sum, a) => sum + a.planned_hours, 0);

    const planned_sprint = monthPlanned
      .filter(a => projectMap.get(a.project_id)?.type === ProjectType.Sprint)
      .reduce((sum, a) => sum + a.planned_hours, 0);
...
export async function computeNPDProjectComparison(
  ...
  for (const project of projects) {
    ...
    // Find planned hours for this project from allocations (sum across engineers)
    const plannedForProject = allocations
      .filter(a => a.project_id === project.project_id)
      .reduce((sum, a) => sum + a.planned_hours, 0);
    groups.get(parent)!.planned_hours += plannedForProject;
...
export async function computeProjectTimeline(
  ...
  for (const month of months) {
    // Sum planned hours across all engineers for this project+month
